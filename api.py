...
class CHCustom():
    def GET(self):
        ...
        if not tanggalmin or not tanggalmax:
            ...
            data=[]
            alldata = Agent._connection.queryAll("SELECT SamplingDate,SamplingTime,Rain FROM {1} WHERE SamplingDate BETWEEN '{2}' and '{3}' and Rain > 0".format(str(pos_ch),tanggalmin,tanggalmax))
            for a in alldata:
